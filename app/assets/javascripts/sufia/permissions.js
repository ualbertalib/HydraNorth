...
Blacklight.onLoad(function() {
  ...
  $('#new_user_name_skel').on('blur', function() {
      ...
      if ( -1 == un.indexOf("@") )
        un += $('.input-group-addon').text()
      var perm = $('#new_user_permission_skel').val();
      if ( $.trim(un).length == 0 ) {
        return;
      }
      $.ajax( {
        url: "/users.json?uq=" + un,
  ...
  $('#add_new_user_skel').on('click', function() {
      ...
      var new_user_name = $('#new_user_name_skel').val()
      if ( -1 == new_user_name.indexOf("@") )
        new_user_name += $('.input-group-addon').text()


      if ( new_user_name == $('#file_owner').data('depositor') ) {
      ...
      if (!is_permission_duplicate( new_user_name )) {
